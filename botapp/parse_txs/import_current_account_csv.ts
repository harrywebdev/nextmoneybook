...
function transformDate(dateString: string) {
  ...
  const timeDefault = (timeChunk: string | undefined) =>
    timeChunk ? timeChunk : "00";
  ...
  const formattedTime = `${timeDefault(timeChunks[0])}:${timeDefault(
    timeChunks[1]
  )}:${timeDefault(timeChunks[2])}`;

  return new Date(`${formattedDate} ${formattedTime}.000Z`);
}

export default async function importCurrentAccountCsv(
  csv: CurrentAccountCsvRow[]
) {
